...
The Cell State Pattern captures a DAO Cell's dynamic state through three core dimensions using the **[nucleus](docs/dao-primitives/implementation/patterns/collaboration-scale-patterns/nucleus.md)** [primitive](data/concepts/primitives.md) - a version-controlled document that tracks group state for both internal coherence and external [coordination](data/concepts/coordination.md).

When combined with the **[circle](docs/dao-primitives/implementation/patterns/collaboration-scale-patterns/circle.md)** primitive (sociocratic team structure), Cell State provides both the documentation framework (nucleus) and the organizational structure (circle) needed for autonomous [teams](data/concepts/teams.md) to function effectively.
...
**Phase**
The cell state document can serve 5 different purposes depending on the [phase](docs/dao-primitives/primitives-framework/concepts/group-phase/index.md) a group is at in its lifecycle:

1. As a collaborative document to start to gather thoughts and intentions for the group in the [conversation](docs/dao-primitives/primitives-framework/concepts/group-phase/conversation-phase.md) phase
2. As a proposal template for a formal proposal to approve the establishment of the team in the [formation](docs/dao-primitives/primitives-framework/concepts/group-phase/formation-phase.md)
3. As the structure within which to develop and record the team's specific agreements and plans during [organization](docs/dao-primitives/primitives-framework/concepts/group-phase/organization-phase.md) phase
4. As the capture of the ongoing evolving current state of the operating group. Visible, public (to the extent that this is practical) interface for those coordinating with the group through [coordination](docs/dao-primitives/primitives-framework/concepts/group-phase/iteration-phase.md)phase
5. As the final record that archives what the group has achieved and who contributed to these achievements through[completion](docs/dao-primitives/primitives-framework/concepts/group-phase/completion-phase.md) phase
...
- **Decision-making**
  ...
  - [Decision-log](docs/dao-primitives/implementation/patterns/collaboration-scale-patterns/decision-log.md) - recording important decisions in a secure way
  - Tools
...
[Cell State Template](docs/dao-primitives/primitives-framework/implementation/cell-state-template.md): Use this template as a starting point for designing an appropriate cell state document for a team. Select categories to include from the Components of Cell State section and base
...
- **Phase of development** - a Cell in an early [phase](docs/dao-primitives/primitives-framework/concepts/group-phase/index.md) of development may require less specific detail, reflecting its lack of formal structure, compared to an mature operational Cell.
...
In the early phases of development of a Cell ([conversation-phase](docs/dao-primitives/primitives-framework/concepts/group-phase/conversation-phase.md) & [formation-phase](docs/dao-primitives/primitives-framework/concepts/group-phase/formation-phase.md)] phase) the Cell's state can stored informally in a simple document editor. However as the group implements its formal structure and agreements with other entities in the network, it enters the [organization-phase](docs/dao-primitives/primitives-framework/concepts/group-phase/organization-phase.md) phase, and its cell state will need to be stored in a way that produces:

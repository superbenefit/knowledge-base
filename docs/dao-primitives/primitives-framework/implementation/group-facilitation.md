...
This guide helps facilitators navigate and use the Primitives Framework to support a group to develop a decentralized [governance](data/concepts/governance.md) and operational structure. It follows steps to assess group context, design appropriate governance and operational structures, and then implement and operationalize these systems. It serves as a roadmap to navigate through the existing framework resources, showing facilitators how to sequence and apply the tools developed here.
...
2. **Design** - [Governance](data/concepts/governance.md) and operational structure using organizational patterns and tools - produce a governance design/proposal
...
For example, setting up a single cell within an existing operational structure could be a relatively fast process because you are dealing with a small collaboration scale team making mostly internal [decisions](data/concepts/decisions.md) about how it will structure and govern itself.

At the other end of the spectrum, however, if you're dealing with a large group looking to establish a DAO, this group facilitation process will be much more involved. The larger group of stakeholders needing to be engaged across multiple different scales will be more complex and time-consuming, and the resulting [governance](data/concepts/governance.md) and operational structure will likewise be more complex.
...
3. **Selecting Patterns**: Choosing appropriate organizational patterns from the [Pattern Library](docs/dao-primitives/implementation/patterns/index.md)
...
For larger groups and those encompassing multiple stakeholder contexts, the sense making process becomes even more involved. These environments contain diverse perspectives, competing incentives, and varying definitions of success that must be surfaced and integrated before effective [governance](data/concepts/governance.md) can be designed.

### Mapping Organizational Context and Stakeholder Landscapes

The heart of group sense making involves systematically mapping the current organizational context, beginning with stakeholder perspectives on vision, [purpose](data/concepts/purpose.md), and impact goals. This mapping process reveals not just what stakeholders want the group to achieve, but how they understand the group's role within larger systems and what success looks like from their vantage point.
...
**For [Community](data/concepts/community.md) Networks/Multi-Stakeholder Groups:**

1. Gather insights from different stakeholder groups separately
2. Synthesize and share insights back to the wider [community](data/concepts/community.md)
...
- **Value Flows**: How value creation and exchange naturally occurs within the [network](data/concepts/networks.md)
- **Power Dynamics**: Where influence and [decision-making](data/concepts/decisions.md) authority currently reside
- **Resource Dependencies**: What the group needs to function and where those [resources](data/concepts/resources.md) come from

Understanding these emergent patterns provides crucial input for the design phase, as [governance](data/concepts/governance.md) structures must align with and support the natural dynamics of the network rather than imposing artificial constraints.

Throughout this process, facilitators can draw upon various sense making tools and methodologies available in the [sense-making-facilitation-tools](docs/dao-primitives/implementation/guides/sense-making-facilitation-tools.md) for specific facilitation needs.

### Producing Discovery Outputs: Reports and Proposals

The discovery phase culminates in concrete outputs that capture insights and provide clear direction for next steps. These outputs serve as both culmination and foundation—capturing the collective intelligence generated through sense making while establishing the legitimacy and direction needed for [governance](data/concepts/governance.md) design.

#### 1. Discovery Report

The discovery report comprehensively documents the sense making process and should include:

- **Shared Understanding**: Clear articulation of agreed-upon [purpose](data/concepts/purpose.md) and high-level goals
...
This report functions as more than documentation—it becomes a foundational artifact that establishes legitimacy for the [governance](data/concepts/governance.md) design process and provides accountability for how stakeholder input shaped the emerging direction.
...
- Initial [group-state-template](docs/dao-primitives/resources/templates/group-state-template.md) capturing [purpose](data/concepts/purpose.md) and high-level goals
- Internal decision to proceed to design phase
- [Resource](data/concepts/resources.md) allocation for design activities
...
When working with larger, more complex groups, the proposal often involves designating a smaller design team that will work in consultation with the broader [community](data/concepts/community.md). This approach balances the need for focused design work with ongoing stakeholder engagement, but requires clear [agreements](data/concepts/agreements.md) about how the design team will maintain connection with and accountability to the wider group throughout the design process.

---

## III. Design

**Design [Governance](data/concepts/governance.md) & Operational Structures**
...
- Gather key stakeholders to define essential [purpose](data/concepts/purpose.md)
- Identify values that [governance](data/concepts/governance.md) must uphold
...
Based on the discovery report, the design team needs to define the overarching parameters of the [governance](data/concepts/governance.md) and operational system. These requirements emerge from the articulated [purpose](data/concepts/purpose.md) and high-level goals, as well as the needs and desires of different stakeholders, the activities they want to engage in, and the constraints they operate within.
...
The [governance](data/concepts/governance.md) system must be designed to handle the specific [decision-making](data/concepts/decisions.md) needs and stakeholder dynamics that emerged during discovery:

- **Decision Scope**: What types of decisions need formal governance processes versus informal [coordination](data/concepts/coordination.md)
...
- **Resource Management**: What [resources](data/concepts/resources.md) governance will oversee and how allocation decisions get made
...
The operational structure must support the group's ability to execute on its [purpose](data/concepts/purpose.md) while maintaining the autonomy and [coordination](data/concepts/coordination.md) patterns that stakeholders value:

- **Core Activities**: How the group will operationalize its purpose to achieve stated goals
- **Autonomy Levels**: The degree of decentralization of authority and [responsibilities](data/concepts/responsibilities.md) needed for operations to succeed
- **Information Systems**: How information needs to flow through the operational structure for effective [coordination](data/concepts/coordination.md)

For small teams with a clear mandate, this requirements mapping can often be completed in focused working sessions. However, for more complex groups involving multiple stakeholder types or scales, this process may require additional engagement and insights gathering from across the [network](data/concepts/networks.md) to ensure all perspectives are captured and integrated.
...
Review the [dao-primitives framework](docs/dao-primitives/primitives-framework/concepts/index.md) along with [index](docs/dao-primitives/implementation/patterns/index.md) and [index](docs/dao-primitives/implementation/case-studies/index.md) to guide the group toward settling on a governance approach that matches their outlined requirements rather than defaulting to popular or familiar options that may not fit their context.

#### 2. Entities and Agreements

The Primitives Framework provides four basic organizational patterns that offer a flexible system for [governance](data/concepts/governance.md) design. These building blocks are designed so governance and operational patterns can be applied to them to compose bespoke yet coherent governance structures that deliver the governance qualities a group desires.
...
The design process involves determining which combination of these patterns will best serve the group's requirements, then identifying specific approaches for how to compose them into functional structures. Examples of using these patterns to compose governance and operational models can be found in [index](docs/dao-primitives/implementation/case-studies/index.md), including case studies like [docs/dao-primitives/primitives-framework/studies/rpp-governance-case-study](docs/dao-primitives/primitives-framework/studies/rpp-governance-case-study.md).

#### 3. Pattern Selection

Based on the chosen [governance](data/concepts/governance.md) approach, the design team builds out a complete governance and operational model by selecting patterns best suited to delivering the desired functionality. This selection process requires balancing multiple considerations: the group's current capacity, their growth projections, the complexity they can manage, and the specific [coordination](data/concepts/coordination.md) challenges they face.

Governance and operational patterns can be found in [index](docs/dao-primitives/implementation/patterns/index.md). Facilitators should help the group select patterns that work effectively in conjunction with the chosen organizational building blocks (DAOs, Cells, Roles & Tasks) to create a governance and operational system that functions as intended rather than creating elegant structures that fail in practice.

#### 4. Feedback and Approval

Meaningful participation from all group stakeholders in deciding on [governance](data/concepts/governance.md) and operational design is essential for both quality outcomes and system legitimacy. For large, complex groups, this participation must be orchestrated across multiple scales to balance broad input with design coherence:

1. **Collaboration Scale**: The team tasked with governance design needs internal processes for evaluating ideas and making design [decisions](data/concepts/decisions.md)
2. **Coordination Scale**: Larger groups of highly engaged individuals can participate through feedback processes, workshops, and other [coordination](data/concepts/coordination.md) mechanisms
3. **Constituency Scale**: Wider [communities](data/concepts/community.md) of stakeholders can engage through preference signaling and voting mechanisms

Managing engagement across different scales ensures the design process benefits from broad [community](data/concepts/community.md) intelligence while preventing the tyranny of structurelessness from derailing effective [decision-making](data/concepts/decisions.md). The goal is to harness collective wisdom without allowing the design process to devolve into protracted politics.

#### 5. Design Proposal

The design phase should conclude with a clear proposal that confirms the [governance](data/concepts/governance.md) and operational model. For small groups, this might take the form of an internal decision recorded in the group's state document. For larger, more complex groups, this typically requires a formal [community](data/concepts/community.md) proposal process.
...
The final component of the design phase involves creating a clear roadmap for implementing the [governance](data/concepts/governance.md) system. This implementation plan must account for the dynamic nature of group development and the reality that complex governance systems evolve rather than launching fully formed.

#### Group-Phase Considerations

Follow the [index](docs/dao-primitives/primitives-framework/concepts/group-phase/index.md) model to ensure implementation steps are appropriate for each phase of the group's evolution:

- **Conversation Phase**: Informal gathering of ideas and intentions
- **Formation Phase**: Basic [agreements](data/concepts/agreements.md) and initial structures
...
For larger, more complex groups, different parts of the [governance](data/concepts/governance.md) and operational structure may exist at different phases simultaneously as the system evolves. Single groups typically exist within one phase at a time, allowing for more straightforward implementation sequencing.

#### Group-Scale Integration

Similarly, follow the group-scale model to ensure the implementation plan addresses governance entities, [agreements](data/concepts/agreements.md), tools, and [practices](data/concepts/practices.md) appropriate to current and projected group scales. Complex groups will have multiple scales represented across their governance structure, with smaller scale groups nested within larger structures. The implementation plan must account for scale evolution as groups grow and change over time.

#### Group-State Anchoring

Anchor the development of the [governance](data/concepts/governance.md) system to the evolution of group-state documentation for each entity in the system. For DAO development, begin with sketching a high-level version of the DAO state appropriate to the formation phase. This can provide essential infrastructure like proposal processes and [treasury](data/concepts/treasury.md) management that enable [community](data/concepts/community.md) stakeholders to provide mandates for ongoing governance development and [resource](data/concepts/resources.md) allocation.
...
Several implementation guides provide documented processes for specific [governance](data/concepts/governance.md) systems. If the chosen governance approach and pattern set maps to available guides, they can be followed for detailed implementation guidance:
...
Produce a comprehensive plan for implementing the [governance](data/concepts/governance.md) and operational system, including:

- **Implementation Phases**: Sequenced stages of system development
- **Timelines**: Realistic schedules for each phase and milestone
- **Budget Requirements**: [Resource](data/concepts/resources.md) needs for implementation activities
- **Implementation Team**: Who will execute the work and their [responsibilities](data/concepts/responsibilities.md)

### 3. Implementation Proposal

The implementation plan itself requires approval through appropriate [governance](data/concepts/governance.md) mechanisms. For small groups with dedicated [resources](data/concepts/resources.md), this can be an internal decision reflected in group state documentation. For larger groups with [community](data/concepts/community.md) membership, this should follow established or new community proposal processes to ensure proper authorization and resource allocation.
...
- [Complete Framework Overview](docs/dao-primitives/primitives-framework/concepts/index.md)
- [Implementation Resources](docs/dao-primitives/implementation/index.md)
- [Case Studies](docs/dao-primitives/implementation/case-studies/index.md) for real-world examples
...
- [Coordination](data/concepts/coordination.md) - Mechanisms enabled by facilitation
- [Teams](data/concepts/teams.md) - Groups benefiting from facilitation
- [Governance](data/concepts/governance.md) - Decision-making supported by facilitation
- [DAO Primitives Framework](docs/dao-primitives/index.md) - Framework using facilitation practices

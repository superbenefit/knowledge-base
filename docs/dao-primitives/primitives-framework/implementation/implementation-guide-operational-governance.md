...
- [Implementation-guide-operational-governance](docs/dao-primitives/primitives-framework/implementation/implementation-guide-operational-governance.md) (this guide)
- [Implementation-guide-multi-stakeholder-governance](docs/dao-primitives/primitives-framework/implementation/implementation-guide-multi-stakeholder-governance.md)
...
- A [community](data/concepts/community.md) or constituency-level [governance](data/concepts/governance.md) structure exists (or is being created) to define [purpose](data/concepts/purpose.md) and high-level direction
...
**Prerequisites:**
- Network [purpose](data/concepts/purpose.md) and high-level goals are established (via community governance)
...
- [DAO](data/concepts/daos.md) contributors working to create more effective [coordination](data/concepts/coordination.md) systems
...
The [DAO primitives framework](docs/dao-primitives/primitives-framework/concepts/index.md) is designed to allow us to compose governance systems that produce purpose-aligned networks of small autonomous teams with powerful organizational dynamics, making these networks effective in solving important problems.
...
Using the [DAO primitives framework](docs/dao-primitives/primitives-framework/concepts/index.md) to create a purpose aligned network of small autonomous teams, there tend to be three facets of governance structure.
...
The operational governance model forms the coordinating backbone of a [DAO](data/concepts/daos.md) network, translating community-defined purpose into effective execution through autonomous teams. It operates at the [coordination and collaboration scales](data/concepts/groups.md#groups-at-different-scales), creating the organizational infrastructure that enables small autonomous teams to work together coherently while maintaining the flexibility and innovation that comes from decentralized decision-making.
...
2. **[Coordination](data/concepts/coordination.md)**: The network of coordinating structures that enable [Cells](data/concepts/roles.md) to work together, share resources, develop strategy, and maintain coherence across the operational network while preserving Cell autonomy.
3. **Cell-Level Governance**: Individual autonomous small teams ([Cells](data/concepts/roles.md)) that have the freedom to determine how they execute their work, make local decisions, and coordinate directly with other Cells as needed for their specific functions.
...
There are many ways to use the [DAO primitives](docs/dao-primitives/primitives-framework/concepts/index.md) to compose a network of operational teams. The particular system you create will reflect the specific needs of the work your community wants to engage in.
...
You may want to or have already created an impact model (or theory of change) to describe the way you want your network to have positive impact on the world. It is not necessary to do this right away, as in the early [phases](data/concepts/groups.md#groups-in-developmental-phases) of developing your governance structure these details may still be emerging. But it is recommended to have a clearly articulated impact model as you move into [coordination and organization phases](data/concepts/groups.md#groups-in-developmental-phases). This will ensure that you have a solid coherent and testable foundation to connect your network's purpose to actual measurable change in the world.
...
2. **Decision Scope**: What types of decisions need operational governance?
   ...
   - [Coordination](data/concepts/coordination.md) between [cells](data/concepts/roles.md)
   - Conflict resolution between teams
3. **Stakeholder Inclusion**: Who needs representation in operational governance?
   - [Cell](data/concepts/roles.md) leads/representatives
   - Team members
...
1. **Highly structured top-down decision-making**
   ...
   - Build hierarchies of [cells](data/concepts/roles.md) that answer to cells above them
...
2. **Highly unconstrained networks**
   ...
   - Free-forming of [cells](data/concepts/roles.md) that coordinate without constraints from the larger network
...
Using the [DAO primitives framework](docs/dao-primitives/primitives-framework/concepts/index.md), design a structure that implements a network of operational [Cells](data/concepts/roles.md). It needs to cover four different levels of scale:

1. **Community governance level ([constituency scale](data/concepts/groups.md#groups-at-different-scales))**
   ...
   - _See [Community Governance](data/concepts/governance.md#community-governanceartifactspatternscommunity-governancemd) pattern and implementation guide_
2. **Cell coordination level ([coordination scale](data/concepts/groups.md#groups-at-different-scales))**
   - How the operational network connects with community governance
     - Perhaps via a top-level [Cell](data/concepts/roles.md) that takes instruction from community governance
   ...
   - Define the initial structure of [Cells](data/concepts/roles.md) needed to instantiate operations
3. **Cell level ([collaboration scale](data/concepts/groups.md#groups-at-different-scales))**
   - Define minimum information standards (state) expected of [Cells](data/concepts/roles.md)
   - Create templates or archetypal [Cell](data/concepts/roles.md) types to standardize new Cells
4. **Partnerships level (network scale)**
   - Design a mechanism for [Cells](data/concepts/roles.md) to partner with other organizations and networks
   - _See [Multi-Stakeholder Governance](data/concepts/governance.md#multi-stakeholder-governance) Implementation Guide_
...
6. **Boundaries and Constraints**: Limits on governance authority and [Cell](data/concepts/roles.md) autonomy
...
2. Set up core [coordination](data/concepts/coordination.md) primitives (basic [agreements](data/concepts/agreements.md) and protocols)
3. Establish foundational [roles](data/concepts/roles.md) and [responsibilities](data/concepts/responsibilities.md)
...
Use the concept of [group phase](data/concepts/groups.md#groups-in-developmental-phases) to implement your governance structure in an evolutionary way, formalizing [Cells](data/concepts/roles.md) and other primitives as they are ready, rather than forcing a completed governance design all at once.

#### Conversation Phase

During this initial [phase](data/concepts/groups.md#groups-in-developmental-phases), focus on:
...
As implementation progresses, the [formation phase](data/concepts/groups.md#groups-in-developmental-phases) includes:

- Formalizing core [agreements](data/concepts/agreements.md), communication and meeting protocols
- Establishing initial [roles](data/concepts/roles.md) and [responsibilities](data/concepts/responsibilities.md)
...
2. Establish critical [roles](data/concepts/roles.md) and select/elect initial occupants
...
In this [phase](data/concepts/groups.md#groups-in-developmental-phases), focus on:
...
- Developing [policies](data/concepts/policies.md) (e.g., codes of conduct, conflict resolution processes)
...
In the final implementation [phase](data/concepts/groups.md#groups-in-developmental-phases), establish:

- Ongoing community engagement/participation in governance
- Ongoing upkeep of documentation and systems
- Ongoing management of governance [roles](data/concepts/roles.md) and authorities
- Continued processes for governance feedback and evolution

**Key Activities:**

1. Implement teams, squads, [Cells](data/concepts/roles.md) and [roles](data/concepts/roles.md) to manage ongoing governance functions
...
If the operational governance design is leveraging the DAO primitives, there will likely be a top-level-cell or other entity that will serve as the anchor for the governance structure (e.g be accountable to the community governance structure, hold legal and administrative [responsibilities](data/concepts/responsibilities.md) etc). In this entity build out a master "state" for the whole operational structure.
...
4. **[Role](data/concepts/roles.md) Descriptions**: Clear documentation of governance [roles](data/concepts/roles.md) and [responsibilities](data/concepts/responsibilities.md)
5. **Process Maps**: Visual representations of key governance processes
6. **[Policies](data/concepts/policies.md)**: Explicit guidelines governing community behavior
...
2. **Tool Selection** Here are some potential tool options for different core operational governance functions: a. **Knowledge repository**
   ...
   - A mechanism for containing and updating [Cell](data/concepts/roles.md) state
...
Use the [group phase framework](data/concepts/groups.md#groups-in-developmental-phases) to implement your governance structure in an evolutionary way, formalizing [Cells](data/concepts/roles.md) and other primitives as they are needed, rather than forcing a completed governance design in one go.
...
- Are [Cells](data/concepts/roles.md) able to operate autonomously while maintaining alignment?
...
2. **[Constituency Scale](data/concepts/groups.md#groups-at-different-scales)**: how decisions at the community governance level influences the operational governance
3. **[Coordination Scale](data/concepts/groups.md#groups-at-different-scales)**: how the operational network makes decisions, allocates resources etc
4. **[Collaboration Scale](data/concepts/groups.md#groups-at-different-scales)**: how individual [Cells](data/concepts/roles.md) make decisions about what work they do and how they do it
...
Effective operational governance is not a fixed structure but an evolving system that grows more sophisticated as your network learns and adapts. By thoughtfully applying the [DAO primitives framework](docs/dao-primitives/primitives-framework/concepts/index.md), organizations can create operational structures that balance autonomy with [coordination](data/concepts/coordination.md), enabling small autonomous teams to work together powerfully while maintaining the flexibility to innovate and respond to changing conditions.

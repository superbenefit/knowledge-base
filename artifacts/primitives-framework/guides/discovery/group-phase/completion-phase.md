...
The Completion Phase marks the intentional closure of a group that has fulfilled its [purpose](tags/purpose.md) or reached a natural endpoint. Following the [Iteration Phase](artifacts/primitives-framework/guides/discovery/group-phase/iteration-phase.md), focus shifts to preserving the group's legacy, ensuring knowledge and resources transition appropriately, and celebrating achievements. Thoughtful completion honors contributions while freeing resources for new opportunities within the [network](tags/networks.md).

Complete this phase by documenting learnings, transitioning assets, and formally closing the group's operations.
...
During the Completion Phase, the [group state](artifacts/primitives-framework/guides/design/group-state.md) documentation evolves into its final form, serving as both a transition tool and a historical record:
